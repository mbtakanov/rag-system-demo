...
class HybridRanker:
    ...
    def search(
        ...
        # vector_results = self.vector_store.search(query, k=k, filter=metadata_filter)
        # bm25_results = self.bm25_store.search(query, k=k, filter=metadata_filter)
        
        # vector_docs = {doc.page_content: (1 - score) for doc, score in vector_results}
        # bm25_docs = {doc.page_content: score for doc, score in bm25_results}
        
        # vector_docs_norm = self._normalize_scores(vector_docs)
        # bm25_docs_norm = self._normalize_scores(bm25_docs)
        
        # combined = {}
        # all_content = set(vector_docs_norm.keys()) | set(bm25_docs_norm.keys())
        
        # for content in all_content:
        #     vec_score = vector_docs_norm.get(content, 0.0)
        #     bm25_score = bm25_docs_norm.get(content, 0.0)
    ...
    def _matches_filter(self, doc: Document, metadata_filter: Optional[Dict[str, str]]) -> bool:
